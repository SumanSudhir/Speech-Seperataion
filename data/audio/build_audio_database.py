...
WAV_REPO_PATH = "../dataset/norm_audio_train"
DATABASE_REPO_PATH = '../dataset/AV_model_database'
FRAME_LOG_PATH = '../dataset/valid_frame_train.txt'
...
def train_test_split(dataset_log_path, data_range=[0, 50000], test_ratio=0.3, shuffle=False,
...
if __name__ == "__main__":
    ...
    dataset_log_path_train = '%s/dataset_train.txt' % DATABASE_REPO_PATH
    dataset_log_path_train_AV = '%s/AVdataset_train.txt' % DATABASE_REPO_PATH

    with open(dataset_log_path_train, 'r') as t:
        lines = t.readlines()
        for line in lines:
            ...
            with open(dataset_log_path_train_AV, 'a') as f:
                f.write(newline)

    dataset_log_path_val = '%s/dataset_val.txt' % DATABASE_REPO_PATH
    dataset_log_path_val_AV = '%s/AVdataset_val.txt' % DATABASE_REPO_PATH

    with open(dataset_log_path_val, 'r') as t:
        lines = t.readlines()
        for line in lines:
            info = line.strip().split('.')
            num1 = info[0].strip().split('-')[1]
            ...
            newline = line.strip() + ' ' + num1 + '_face_emb.npy' + ' ' + num2 + '_face_emb.npy\n'
            with open(dataset_log_path_val_AV, 'a') as f:
                f.write(newline)

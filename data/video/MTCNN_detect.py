...
import torch_mtcnn as detector
import cv2
import pandas as pd
import os
import glob
...
def bounding_box_check(faces, x, y):
    # check the center
    for face in faces:
        bounding_box = list(map(int, face[:-1]))
        if bounding_box[1] < 0:
            bounding_box[1] = 0
        if bounding_box[0] < 0:
            bounding_box[0] = 0
    ...
    for i in range(detect_range[0], detect_range[1]+1):
        ...
        for j in range(1, 76):
            ...
            if not os.path.exists('%s/%s' % (frame_path, file_name)):
                ...
                continue
            face_detect(file_name, frame_path, train_data_csv)
    ...
    for i in range(detect_range[0], detect_range[1]+1):
        for j in range(1, 76):
            ...
            if not os.path.exists('%s/%s' % (frame_path, file_name)):
                print('cannot find input: ' + '%s/%s' % (frame_path, file_name))
                continue
            face_detect(file_name, frame_path, test_data_csv)
